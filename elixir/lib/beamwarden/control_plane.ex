defmodule Beamwarden.ControlPlane do
  @moduledoc false

  alias Beamwarden.{Cluster, PortingTask}
  ...
  def ensure_local_session(session_id) do
    case Registry.lookup(Beamwarden.SessionRegistry, session_id) do
      [{pid, _value}] ->
        Beamwarden.ClusterDaemon.claim_local_owner(:session, session_id)
        {:ok, pid}

      [] ->
        DynamicSupervisor.start_child(
          Beamwarden.SessionSupervisor,
          {Beamwarden.SessionServer, session_id}
        )
        |> normalize_start()
    end
  end
  ...
  def ensure_local_workflow(workflow_id) do
    case Registry.lookup(Beamwarden.WorkflowRegistry, workflow_id) do
      [{pid, _value}] ->
        Beamwarden.ClusterDaemon.claim_local_owner(:workflow, workflow_id)
        {:ok, pid}

      [] ->
        DynamicSupervisor.start_child(
          Beamwarden.WorkflowSupervisor,
          {Beamwarden.WorkflowServer, workflow_id}
        )
        |> normalize_start()
    end
  end
  ...
  def submit_prompt_local(session_id, prompt) do
    with {:ok, _pid} <- ensure_local_session(session_id) do
      {:ok, Beamwarden.SessionServer.submit(session_id, prompt)}
    end
  end

  def session_snapshot_local(session_id) do
    with {:ok, _pid} <- ensure_local_session(session_id) do
      {:ok, Beamwarden.SessionServer.snapshot(session_id)}
    end
  end

  def list_local_sessions do
    Beamwarden.SessionRegistry
    |> Registry.select([{{:"$1", :_, :_}, [], [:"$1"]}])
    |> Enum.map(&Beamwarden.SessionServer.snapshot/1)
  end

  def add_workflow_step_local(workflow_id, title, description \\ nil) do
    with {:ok, _pid} <- ensure_local_workflow(workflow_id) do
      {:ok, Beamwarden.WorkflowServer.add_step(workflow_id, title, description)}
    end
  end

  def complete_workflow_step_local(workflow_id, step_id) do
    with {:ok, _pid} <- ensure_local_workflow(workflow_id) do
      {:ok, Beamwarden.WorkflowServer.complete_step(workflow_id, step_id)}
    end
  end

  def workflow_snapshot_local(workflow_id) do
    with {:ok, _pid} <- ensure_local_workflow(workflow_id) do
      {:ok, Beamwarden.WorkflowServer.snapshot(workflow_id)}
    end
  end
  ...
  def start_workflow_local(workflow_id, tasks \\ []) do
    with {:ok, _pid} <- ensure_local_workflow(workflow_id) do
      Enum.each(tasks, fn
        %PortingTask{} = task ->
          Beamwarden.WorkflowServer.add_step(workflow_id, task.title || task.id, task.description)

        description when is_binary(description) ->
          Beamwarden.WorkflowServer.add_step(workflow_id, description)
      end)
  ...
  def advance_task_local(workflow_id, task_id, status, detail \\ nil) do
    with {:ok, _pid} <- ensure_local_workflow(workflow_id) do
      {:ok, Beamwarden.WorkflowServer.transition_step(workflow_id, task_id, status, detail)}
    end
  end
  ...
  def list_local_workflows do
    ...
    |> Registry.select([{{:"$1", :_, :_}, [], [:"$1"]}])
    |> Enum.map(&Beamwarden.WorkflowServer.snapshot/1)
  end
  ...
  def session_running?(session_id),
    do: registry_running?(Beamwarden.SessionRegistry, session_id)

  def workflow_running?(workflow_id),
    do: registry_running?(Beamwarden.WorkflowRegistry, workflow_id)
  ...
  defp resolve_session_node(session_id) do
    find_running_owner(session_id, :session) ||
      ...
      Beamwarden.ClusterDaemon.resolve_owner(
        :session,
        session_id,
        stored_owner_node(Beamwarden.SessionStore.owner_node(session_id))
      )
  end
  ...
  defp resolve_workflow_node(workflow_id) do
    find_running_owner(workflow_id, :workflow) ||
      ...
      Beamwarden.ClusterDaemon.resolve_owner(
        :workflow,
        workflow_id,
        stored_owner_node(Beamwarden.WorkflowStore.owner_node(workflow_id))
      )
  end
  ...
  defp daemon_preferred_node do
    if Beamwarden.Daemon.current_server?(), do: node(), else: nil
  end
