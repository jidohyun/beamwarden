defmodule Beamwarden.Daemon do
  @moduledoc false
    ...
    cond do
      ...
      true ->
        case ensure_daemon_connection() do
          ...
          {:ok, daemon} ->
            case Cluster.rpc_call(daemon, Beamwarden.CLI, :run_local, [args]) do
  ...
  def start_server(opts \\ []) do
    with :ok <- ensure_server_distribution(opts) do
      Beamwarden.AppIdentity.put_env(:daemon_node, Atom.to_string(node()))
      maybe_put_cookie(opts[:cookie])
      Beamwarden.ClusterDaemon.reconcile_local_runtime()
      {:ok, status_report()}
    end
  end
