defmodule ClawCode.RoutedMatch do
  @moduledoc false
  defstruct [:kind, :name, :source_hint, :score]
end

defmodule ClawCode.RuntimeSession do
  @moduledoc false
  ...
  def as_markdown(%__MODULE__{} = session) do
    [
      ...
      "",
      "## Context",
      ClawCode.Context.render(session.context),
      "",
      "## Setup",
      ...
      "",
      ClawCode.HistoryLog.as_markdown(session.history)
    ]
    |> List.flatten()
    |> Enum.join("\n")
  end
...
defmodule ClawCode.Runtime do
  ...
  def route_prompt(prompt, limit \\ 5) do
    ...
    by_kind = %{
      "command" => collect_matches(tokens, ClawCode.Commands.ported_commands(), "command"),
      "tool" => collect_matches(tokens, ClawCode.Tools.ported_tools(), "tool")
    }
  ...
  def bootstrap_session(prompt, opts \\ []) do
    ...
    context = ClawCode.Context.build()
    setup_report = ClawCode.Setup.run()
    history = %HistoryLog{}
    ...
    history =
      HistoryLog.add(
        history,
        "registry",
        "commands=#{length(ClawCode.Commands.ported_commands())}, tools=#{length(ClawCode.Tools.ported_tools())}"
      )
    ...
    command_messages =
      Enum.map(Enum.filter(matches, &(&1.kind == "command")), fn match ->
        ClawCode.Commands.execute_command(match.name, prompt).message
      end)
    ...
    %RuntimeSession{
      ...
      system_init_message: ClawCode.SystemInit.build(true),
      history: history,
