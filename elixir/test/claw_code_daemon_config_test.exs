defmodule ClawCodeDaemonConfigTest do
  ...
  setup do
    previous_node = ClawCode.AppIdentity.get_env(:daemon_node)
    ...
    on_exit(fn ->
      if previous_node == nil do
        ...
        ClawCode.AppIdentity.put_env(:daemon_node, previous_node)
      end
  ...
  test "uses longnames when the configured daemon host is fully qualified" do
    ClawCode.AppIdentity.put_env(:daemon_node, "claw_code_daemon@daemon.example.internal")
    System.delete_env("CLAW_DAEMON_NAME_MODE")
    System.delete_env("BEAMWARDEN_DAEMON_NAME_MODE")
  ...
  test "environment override forces longnames" do
    ClawCode.AppIdentity.put_env(:daemon_node, "claw_code_daemon@daemon")
  ...
  test "future app env is visible through the compatibility helper" do
    ...
    assert ClawCode.AppIdentity.get_env(:daemon_node) == "beamwarden_daemon@app-env-host"
    assert ClawCode.Daemon.configured_node_label() == "beamwarden_daemon@app-env-host"
  end
